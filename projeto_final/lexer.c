...
bool Acept[4] = {false, false, true, true};
// int states[3][3] ={
//                     {1, 3, 3}, 
//                     {1, 1, 2}, 
//                     {3, 3, 3}
...
TokenType resolve_token_type(int final_state){
    switch (final_state)
    {
    case 2:
        return ID;
...
TokenType get_token(FILE * file){
    ...
    if(current_char == EOF) end_of_file = true;
    ...
    while (Acept[current_state] == false && error == false){
        ...
        if(Advance[current_state][get_char_state(current_char)]) {
            ...
            if (current_char == EOF) {
                current_state = states[current_state][get_char_state(EOF)];
                break;
    ...
    if(!Acept[current_state]) return ERROR;//printf("%c", current_char);
    // else if(check_for_key_word(lexeme)) printf("%s", lexeme);
    else return resolve_token_type(current_state); //printf("ID (%s)", lexeme);
